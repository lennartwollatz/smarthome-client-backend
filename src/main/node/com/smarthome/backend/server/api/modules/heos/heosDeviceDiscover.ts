...
export abstract class HeosDeviceDiscover extends ModuleDeviceDiscover<HeosDeviceDiscovered> {
  protected manufacturer: string;
  protected devices = new Map<string, HeosDeviceDiscovered>();
  ...
  protected constructor(databaseManager: DatabaseManager, manufacturer: string) {
    super(databaseManager);
    this.manufacturer = manufacturer;
  }

  abstract getModuleName(): string;
  abstract getDiscoveredDeviceTypeName(): string;
  ...
  private tryCreateDevice(serviceName: string, service: ServiceCache) {
    ...
    const device = new HeosDeviceDiscovered(
      udn,
      friendlyName,
      modelName,
      modelNumber,
  ...
  async discoverDevicesWithPlayerIds(searchDurationMs: number) {
    return this.startDiscovery(searchDurationMs / 1000);
  }

  private async getPlayerInfo(snapshot: HeosDeviceDiscovered[]) {
    const devicesWithPlayerIds: HeosDeviceDiscovered[] = [];
  ...
  protected async setPlayerInfo(device: HeosDeviceDiscovered) {
