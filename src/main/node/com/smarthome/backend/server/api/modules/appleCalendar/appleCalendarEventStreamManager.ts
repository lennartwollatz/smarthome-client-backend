...
export class AppleCalendarEventStreamManager extends ModuleEventStreamManager<AppleCalendarDeviceController, AppleCalendarEvent> {
  ...
  constructor(managerId: string, moduleId: string, controller: AppleCalendarDeviceController, deviceManager: DeviceManager) {
    ...
    switch(type) {
      case "cal-name-changed":
        if(event.calendarId && event.calendarName) {
          ...
          this.deviceManager.saveDevice(calendarDevice);
        }
        break;
      case "cal-deleted":
        if(event.calendarId) {
