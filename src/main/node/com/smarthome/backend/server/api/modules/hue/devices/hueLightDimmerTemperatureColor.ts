...
export class HueLightDimmerTemperatureColor extends DeviceLightDimmerTemperatureColor {
  ...
  protected async executeSetColor(x: number, y: number): Promise<void> {
    ...
    await this.hueDeviceController.setColor(this.id, x, y);
  }
  ...
  protected async executeSetTemperature(temperature: number): Promise<void> {
    ...
    await this.hueDeviceController.setTemperature(this.id, temperature);
  }
